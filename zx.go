	...
	case Dm_build_1072:
		{
			...
			if conn.dmConnector.rwHA && curConn == conn.rwInfo.connStandby && len(curConn.lastExecInfo.rsDatas) == 0 {
				turnToPrimary = true
			}
		}
	}
	...
	case Dm_build_1072:
		{
			...
			if stmt.dmConn.dmConnector.rwHA && curStmt == stmt.rwInfo.stmtStandby && len(curStmt.execInfo.rsDatas) == 0 {
				turnToPrimary = true
			}
		}
	}
