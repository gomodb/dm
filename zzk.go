...
func (dc *DmConnection) escape(sql string, keywords []string) (string, error) {
	...
	if len(keywords) == 0 && !strings.Contains(sql, "{") {
		return sql, nil
	}
...
func (dc *DmConnection) execOpt(sql string, optParamList []OptParameter, serverEncoding string) (string, []OptParameter, error) {
	...
	if !util.StringUtil.EqualsIgnoreCase(firstWord, "INSERT") && !util.StringUtil.EqualsIgnoreCase(firstWord, "SELECT") && !util.StringUtil.EqualsIgnoreCase(firstWord, "UPDATE") && !util.StringUtil.EqualsIgnoreCase(firstWord, "DELETE") {
		return sql, optParamList, nil
	}
