...
func (rpv *rsPoolValue) refreshed(conn *DmConnection) (bool, error) {
	...
	tss, err := conn.Access.Dm_build_847(any(rpv.m_TbIds).([]uint32))
	if err != nil {
		return false, err
	}
	rpv.m_lastChkTime = time.Now().Nanosecond()
...
func namedValueToValue(stmt *DmStatement, named []driver.NamedValue) ([]driver.Value, error) {
	...
	for i := range dargs {
		...
		for _, nv := range named {
			if nv.Name != "" && strings.EqualFold(nv.Name, stmt.serverParams[i].name) {
				dargs[i] = nv.Value
				found = true
				break
			}
...
func (stmt *DmStatement) executeBatchByRow(args []driver.Value) (err error) {
	...
	for i := 0; i < count; i++ {
		tmpExecInfo, err := stmt.dmConn.Access.Dm_build_804(stmt, args[i].([]any), stmt.preExec || i != 0)
		if err == nil {
